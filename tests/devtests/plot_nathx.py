        ...
        if sim.yearvec[sim.t] == self.start_year:
            ...
            if len(inf_inds):
                ...
                for idd, dd in enumerate(self.durations):
                    ...
                    persisted = ~dead & ~cleared & ~(time_to_cin <= (dd+self.interval)) & ~(time_to_cancer <= (dd+self.interval))# Haven't yet cleared or progressed
...
def plot_nh(sim=None):
    ...
    for ln,layer in enumerate(layers):
        axes[0].fill_between(df['years'], bottom, bottom+df[layer], color=colors[ln], label=labels[ln])
        bottom += df[layer]
    ...
    for ln,layer in enumerate(layers):
        axes[1].fill_between(df2['years'], bottom, bottom+df2[layer], color=colors[ln], label=labels[ln])
        bottom += df2[layer]
